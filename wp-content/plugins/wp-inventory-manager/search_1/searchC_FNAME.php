...
$link = '%' . $link . '%';
   ...
   // $table_name = $wpdb->prefix . 'wpinventory_stock';
    $categories = $wpdb->get_results('SELECT Distinct c_fname FROM wp_wpinventory_item WHERE c_fname LIKE "' . $link . '"' . $where .' ORDER BY c_fname ASC ');
