...
class WPIMItem extends WPIMDB {
	...
	public function getstockQty($s_name, $s_m_no, $s_rate, $c_id, $gid) {
		return $this->wpdb->get_var ( "Select s_qty from wp_netrastock_item where stock_name='" . $s_name . "' AND s_model_no='" . $s_m_no . "' AND s_rate=" . $s_rate . " AND category_id=" . $c_id . " AND gid=" . $gid );
	}
	public function getRate($s_name, $s_m_no, $c_id, $gid) {
		//echo '<script>alert( "Hello!!!" );</script>';
		$datas = $this->wpdb->get_results ( "Select s_qty, s_rate from wp_netrastock_item where stock_name='" . $s_name . "' AND s_model_no='" . $s_m_no . "' AND category_id=" . $c_id . " AND gid=" . $gid . " ORDER BY s_rate DESC" );
		foreach ( $datas as $data ) {
			echo '<script>alert( "Current Stock Qty: ' . $data->s_qty . '" );</script>';
			if ($data->s_qty > 0) {
				echo '<script>alert( "if ..Current Rate: ' . $data->s_rate . '" );</script>';
				return $data->s_rate;
				break;
			}
		}
	}
	public function getupdateStockQuery($qty, $s_name, $s_m_no, $s_rate, $c_id, $gid) {
		$stQuery = "Update " . $this->wpdb->prepare ( " wp_netrastock_item SET s_qty = %d", $qty );
		$stQuery = $stQuery . $this->wpdb->prepare ( ' WHERE stock_name=%s AND s_model_no=%s AND s_rate=%d AND category_id=%d AND gid=%d ', $s_name, $s_m_no, $s_rate, $c_id, $gid );
	...
	public function getFrameQuery($order_no, $f_brand1, $f_model1, $f_size1, $f_color1, $f_sp1, $f_rate1, $f_nos, $gid) {
		$query1 = $this->wpdb->prepare ( " " . $this->frame_table . " SET
			...
			f_sp = %d,
                        f_rate = %d,
                        f_nos = %d,
                        gid = %d", $order_no, $f_brand1, $f_model1, $f_size1, $f_color1, $f_sp1, $f_rate1, $f_nos, $gid );
	...
	public function getLensQuery($order_no, $l_brand1, $l_model1, $l_rate1, $r1, $r_size1, $r_tint1, $l1, $l_size1, $l_tint1, $spec1, $note1, $l_sp1, $l_nos, $gid) {
		$queryL1 = $this->wpdb->prepare ( " " . $this->lens_table . " SET
			...
			l_size = %d,
                        ...
                        l_sp = %d,
                        l_nos = %d,
                        gid = %d", $order_no, $l_brand1, $l_model1, $l_rate1, $r1, $r_size1, $r_tint1, $l1, $l_size1, $l_tint1, $spec1, $note1, $l_sp1, $l_nos, $gid );
		return $queryL1;
	}
	...
	public function save($data) {
		...
		$query = $this->wpdb->prepare ( " " . $this->inventory_table . " SET
			...
			gid = %d", $order_no, $c_no, $date, $d_date, $c_fname, $c_lname, $c_gender, $c_add, $c_city, $c_city_pin, $c_email, $c_birth, $c_anni, $f_count, $l_count, $f_price, $l_price, $others, $adj, $total, $adv, $bal, $status, $r_d_sph, $r_d_cyl, $r_d_axis, $r_d_add, $r_d_va, $r_n_sph, $r_n_cyl, $r_n_axis, $r_n_add, $r_n_va, $l_d_sph, $l_d_cyl, $l_d_axis, $l_d_add, $l_d_va, $l_n_sph, $l_n_cyl, $l_n_axis, $l_n_add, $l_n_va, $r_lpd, $l_lpd, $o_desc, $ref_by, $category_id, $inventory_slug, $inventory_sort_order, $now, $gid );
		...
		if ($inventory_id) {
					...
					if ($status == "Delivered" || $getdbstatus == "Delivered") {
						if ($getdbstatus == "Delivered") {
							...
							foreach ( $getdbdatas1 as $getdbdata1 ) {
								$dbdata2 [] = $getdbdata1->f_brand;
								$dbdata2 [] = $getdbdata1->f_model;
								$dbdata2 [] = $getdbdata1->f_rate;
							}
							if ($dbdata2 [0] == $f_brand1 && $dbdata2 [1] == $f_model1 && $dbdata2 [2] == $f_rate1) {
							}  // delivered
					...
					if ($status == "Delivered" || $getdbstatus == "Delivered") {
						if ($getdbstatus == "Delivered") {
							...
							foreach ( $getdbdatas2 as $getdbdata2 ) {
								$dbdata3 [] = $getdbdata2->f_brand;
								$dbdata3 [] = $getdbdata2->f_model;
								$dbdata3 [] = $getdbdata2->f_rate;
							}
							if ($dbdata3 [0] == $f_brand2 && $dbdata3 [1] == $f_model2 && $dbdata3 [2] == $f_rate2) {
							}  // delivered
							                                                                                 // or may be a new update
							else {
								if ($dbdata3 [0] != "") {
				...
				if ($status == "Canceled" && $getdbstatus == "Delivered") {
					...
					if ($status == "Delivered" || $getdbstatus == "Delivered") {
						if ($getdbstatus == "Delivered") {
							...
							if ($dbdata3 [0] == $f_brand3 && $dbdata3 [1] == $f_model3 && $dbdata3 [2] == $f_rate3) {
							}  // delivered
							                                                                                 // or may be a new update
							else {
								if ($dbdata3 [0] != "") {
					...
					if ($status == "Delivered" || $getdbstatus == "Delivered") {
						if ($getdbstatus == "Delivered") {
							...
							foreach ( $getdbdatas3 as $getdbdata3 ) {
								$dbdata4 [] = $getdbdata3->f_brand;
								$dbdata4 [] = $getdbdata3->f_model;
								$dbdata4 [] = $getdbdata3->f_rate;
							}
							if ($dbdata4 [0] == $f_brand4 && $dbdata4 [1] == $f_model4 && $dbdata4 [2] == $f_rate4) {
							}  // delivered
							                                                                                 // or may be a new update
							else {
								if ($dbdata4 [0] != "") {
			...
			if ($r0 != "" && $l0 != "") {
				...
				if ($status == "Canceled" && $getdbstatus == "Delivered") {
					...
					foreach ( $getdbdatas0 as $getdbdata0 ) {
						$l_dbdata0 [] = $getdbdata0->l_brand;
						$l_dbdata0 [] = $getdbdata0->l_model;
						$l_dbdata0 [] = $getdbdata0->l_rate;
					}
					...
					if ($status == "Delivered" || $getdbstatus == "Delivered") {
						if ($getdbstatus == "Delivered") {
							...
							if ($l_dbdata0 [0] == $l_brand0 && $l_dbdata0 [1] == $l_model0 && $l_dbdata0 [2] == $l_rate0) {
							}  // delivered
							                                                                                       // or may be a new update
							else {
								if ($l_dbdata0 [0] != "") {
			...
			if ($r1 != "" && $l1 != "") {
				...
				if ($status == "Canceled" && $getdbstatus == "Delivered") {
					...
					foreach ( $getdbdatas1 as $getdbdata1 ) {
						$l_dbdata1 [] = $getdbdata1->l_brand;
						$l_dbdata1 [] = $getdbdata1->l_model;
						$l_dbdata1 [] = $getdbdata1->l_rate;
					}
					...
					if ($status == "Delivered" || $getdbstatus == "Delivered") {
						if ($getdbstatus == "Delivered") {
							...
							if ($l_dbdata1 [0] == $l_brand1 && $l_dbdata1 [1] == $l_model1 && $l_dbdata1 [2] == $l_rate1) {
							}  // delivered
							                                                                                       // or may be a new update
							else {
								if ($l_dbdata1 [0] != "") {
			...
			if ($r2 != "" && $l2 != "") {
				...
				if ($status == "Canceled" && $getdbstatus == "Delivered") {
					...
					foreach ( $getdbdatas2 as $getdbdata2 ) {
						$l_dbdata2 [] = $getdbdata2->l_brand;
						$l_dbdata2 [] = $getdbdata2->l_model;
						$l_dbdata2 [] = $getdbdata2->l_rate;
					}
					...
					if ($status == "Delivered" || $getdbstatus == "Delivered") {
						if ($getdbstatus == "Delivered") {
							...
							if ($l_dbdata2 [0] == $l_brand2 && $l_dbdata2 [1] == $l_model2 && $l_dbdata2 [2] == $l_rate2) {
							}  // delivered
							                                                                                       // or may be a new update
							else {
								if ($l_dbdata2 [0] != "") {
			...
			if ($r3 != "" && $l3 != "") {
				...
				if ($status == "Canceled" && $getdbstatus == "Delivered") {
					...
					foreach ( $getdbdatas3 as $getdbdata3 ) {
						$l_dbdata3 [] = $getdbdata3->l_brand;
						$l_dbdata3 [] = $getdbdata3->l_model;
						$l_dbdata3 [] = $getdbdata3->l_rate;
					}
					...
					if ($status == "Delivered" || $getdbstatus == "Delivered") {
						if ($getdbstatus == "Delivered") {
							...
							if ($l_dbdata3 [0] == $l_brand3 && $l_dbdata3 [1] == $l_model3 && $l_dbdata3 [2] == $l_rate3) {
							}  // delivered
							                                                                                       // or may be a new update
							else {
								if ($l_dbdata3 [0] != "") {
			...
			if ($r4 != "" && $l4 != "") {
				...
				if ($status == "Canceled" && $getdbstatus == "Delivered") {
					...
					foreach ( $getdbdatas4 as $getdbdata4 ) {
						$l_dbdata4 [] = $getdbdata4->l_brand;
						$l_dbdata4 [] = $getdbdata4->l_model;
						$l_dbdata4 [] = $getdbdata4->l_rate;
					}
					...
					if ($status == "Delivered" || $getdbstatus == "Delivered") {
						if ($getdbstatus == "Delivered") {
							...
							if ($l_dbdata4 [0] == $l_brand4 && $l_dbdata4 [1] == $l_model4 && $l_dbdata4 [2] == $l_rate4) {
							}  // delivered
							                                                                                       // or may be a new update
							else {
								if ($l_dbdata4 [0] != "") {
			...
			if ($f_model0 != "") {
				$f_nos = 0;
				$c_id = 1;
				$f_rate0 = $this->getRate ( $f_brand0, $f_model0, $c_id, $gid );
				echo '<script>alert( "Current Rate: ' . $f_rate0 . '" );</script>';
				$query0 = $this->getFrameQuery ( $order_no, $f_brand0, $f_model0, $f_size0, $f_color0, $f_sp0, $f_rate0, $f_nos, $gid );
				...
				if ($status == "Delivered") {
					...
					$dbs_qty = $this->getstockQty ( $f_brand0, $f_model0, $f_rate0, $c_id, $gid );
					...
					if ($dbs_qty !== null && ($dbs_qty - 1) > 0) {
						...
						// $stQuery=$stQuery . $this->wpdb->prepare(' WHERE stock_name=%s AND s_model_no=%s AND s_rate=%d', $f_brand0, $f_model0, $f_rate0);
						//
						$stQuery = $this->getupdateStockQuery ( $dbs_qty, $f_brand0, $f_model0, $f_rate0, $c_id, $gid );
			...
			$query = 'INSERT INTO' . $query . $this->wpdb->prepare ( ", user_id = %d, plstats = %d,
				inventory_date_added = %s", $user_id, $plstats, $now );
		}
	 ...
	 * 
	 * @param integer $inventory_id
	 *        	- id of the relevant inventory item
	 * @param array $images
	 *        	- array of image urls
	 * @param array $sort
	 *        	- array of indexes for sort order
	 */
