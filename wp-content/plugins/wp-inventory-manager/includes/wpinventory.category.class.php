...
class WPIMCategory extends WPIMDB {
	...
	*/
    ...
    //my code to get a drop down for the models/////////////////////////////////////////////////////////////////////////////////////my code/////////////////////
        
        	public function get_allmodel($args = NULL) {
		
		$categories = $this->wpdb->get_results('SELECT DISTINCT model FROM ' . $this->stock_table . ' where category="Frame" AND gid=1 AND brand="Sirus"');
		return $categories;
	}
        
        	public function modeldropdown() {
		$cat_values = $this->get_allmodel();    
		$select = '<select name="thelist" id="thelist" onChange="combo();"';
		$select.= ($class) ? ' class="' . $class . '"' : '';
		$select.= '>' . PHP_EOL;
		$select.= '<option value="">' . $this->__('Select Model') . '</option>' . PHP_EOL;
		foreach($cat_values AS $category) {
			$select.= '<option value="' . $category->model . '"';
			$select.= ($category->model == $selected) ? ' selected' : '';
			$select.= '>' . $category->model . '</option>' . PHP_EOL;
                //s $output = "<script>console.log( 'Debug Objects: " . $category->model . "' );</script>";
                // echo $output;
		}
		$select.= '</select>' . PHP_EOL;
		return $select;
