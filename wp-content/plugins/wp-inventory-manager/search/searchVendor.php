...
*/
   ...
   // using a single database
    $categories = $wpdb->get_results('SELECT Distinct vendor_name FROM wp_netracustomer_item WHERE vendor_name LIKE "' . $link . '"' . $where .' ORDER BY vendor_name ASC ');
foreach($categories AS $category) {
			$data[]= $category->vendor_name;
                           // $output = "<script>console.log( 'Debug Objects: " . $category->model . "' );</script>";
                        //echo $output;
                        //echo ''.$category->model;
