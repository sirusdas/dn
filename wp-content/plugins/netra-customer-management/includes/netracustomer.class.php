...
abstract class NCMCore {

	const VERSION = '1.2.0';

	const SHORTCODE = 'netracustomer';

	const SETTINGS = 'netracustomer_settings';
	const SETTINGS_GROUP = 'netracustomer_settings_group';

	const VIEWFOLDER = 'netracustomer/views/';
	const LANG = 'netracustomer';
	const MENU = 'purchase';
	const NONCE_ACTION = 'netracustomer_&%k2s$%#!@#8vY^';
	...
	public function __construct() {
		...
		self::$label        = NCMLabel::getInstance();
	...
	public static function store_page_state( $is_single = FALSE ) {
		...
		if ( $update_transient ) {
			...
			$_SESSION['netracustomer_page_state'] = $page_state;
		}
	}

	public static function get_page_state() {
		return ( isset( $_SESSION['netracustomer_page_state'] ) ) ? (array) $_SESSION['netracustomer_page_state'] : array();
	}
...
//	}
	...
	public static function get_reg_info() {
		$reg_info = get_option( 'ncm_license' );

		if ( $reg_info && ! isset( $reg_info['core'] ) ) {
			$reg_info = array( 'core' => $reg_info );
			update_option( 'ncm_license', $reg_info );
		}
	...
	protected static function get_add_ons( $installed = FALSE, $force = FALSE ) {
		$add_ons = get_transient( 'ncm_add_ons' );
		...
		if ( ! $add_ons ) {
			$add_ons = NCMAPI::make_call( 'get_add_ons' );
			set_transient( 'ncm_add_ons', $add_ons, 12 * HOUR_IN_SECONDS );
		}

		$add_ons = apply_filters( 'ncm_add_ons_list', $add_ons );
	...
	protected static function user_can_edit( $inventory_id = NULL ) {
		return NCMAdmin::check_permission( 'edit_item', $inventory_id );
	}
	...
	protected static function check_permission( $type, $inventory_id ) {
		...
		return apply_filters( 'ncm_check_permission', TRUE, $type, $inventory_item );
	}
