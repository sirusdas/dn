 ...
 *
 */
class NCMShortcode extends NCMCore {
	...
	public function get($args = array()) {
		...
		if (self::$args['category_id'] && ! (int)self::$args['category_id']) {
			$NCMCategory = new NCMCategory();
			$category = $NCMCategory->get_id_from_name(self::$args['category_id']);
			if ($category) {
		...
		self::$loop = new NCMLoop(self::$args);
		netracustomer_set_loop(self::$loop);
	...
	public function get_template($template, $echo = FALSE, $type = 'list') {
		...
		if ( ! file_exists($file)) {
			echo '<!-- NETRACustomer Error: template part ' . $file . ' missing! -->';
		}
		...
		do_action('ncm_get_template', $template, $echo, $type, self::$inventory_id);
