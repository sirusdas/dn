...
abstract class NSMCore {

	const VERSION = '1.2.0';

	const SHORTCODE = 'netrastock';

	const SETTINGS = 'netrastock_settings';
	const SETTINGS_GROUP = 'netrastock_settings_group';

	const VIEWFOLDER = 'netrastock/views/';
	const LANG = 'netrastock';
	const MENU = 'netrastock';
	const NONCE_ACTION = 'netrastock_&%k2s$%#!@#8vY^';
	...
	public static function store_page_state( $is_single = FALSE ) {
		...
		if ( $update_transient ) {
			...
			$_SESSION['netrastock_page_state'] = $page_state;
		}
	}

	public static function get_page_state() {
		return ( isset( $_SESSION['netrastock_page_state'] ) ) ? (array) $_SESSION['netrastock_page_state'] : array();
	}
...
/* my code this has to be deleted in future
...
//	}
	...
	public static function get_reg_info() {
		$reg_info = get_option( 'NSM_license' );

		if ( $reg_info && ! isset( $reg_info['core'] ) ) {
			$reg_info = array( 'core' => $reg_info );
			update_option( 'NSM_license', $reg_info );
		}
	...
	protected static function get_add_ons( $installed = FALSE, $force = FALSE ) {
		$add_ons = get_transient( 'NSM_add_ons' );
		...
		if ( ! $add_ons ) {
			$add_ons = NSMAPI::make_call( 'get_add_ons' );
			set_transient( 'NSM_add_ons', $add_ons, 12 * HOUR_IN_SECONDS );
		}

		$add_ons = apply_filters( 'NSM_add_ons_list', $add_ons );
	...
	protected static function user_can_edit( $inventory_id = NULL ) {
		return NSMAdmin::check_permission( 'edit_item', $inventory_id );
	}
	...
	protected static function check_permission( $type, $inventory_id ) {
		...
		return apply_filters( 'NSM_check_permission', TRUE, $type, $inventory_item );
	}
