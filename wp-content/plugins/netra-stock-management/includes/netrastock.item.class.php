	...
	*/
	...
	public function default_args() {
		$defaults = array(
				...
				'page_size'			=> NSMCore::$config->get('page_size'),  // Unlimited
				'page'				=> 0,  		// Beginning
	...
	public function get_all($args = NULL, $get_counts = FALSE) {
		...
		$args = apply_filters('NSM_query_item_args', $args);
...
//		 echo '<br>' . $query;
		...
		return apply_filters('NSM_get_items', $items);
	} 
	...
	public function get_pdata($iv_no) {
		return $this->wpdb->get_results($this->wpdb->prepare('SELECT * FROM wp_netrastock_data WHERE invoice_no = %s ORDER BY  p_count ASC', $iv_no));
	}        
	/**
	...
	public function save_reserve($inventory_id, $quantity) {
		...
		do_action('NSM_save_reserve', $inventory_id, $quantity);
