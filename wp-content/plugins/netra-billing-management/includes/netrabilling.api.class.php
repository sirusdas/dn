...
class NBMAPI {
	...
	const API_URL = 'http://www.netrabilling.com/license_api/'; // 'http://netrabilling.mrwpress.com'; //
	
	const REG_ITEM_NAME = 'NETRA Billing Manager';
	...
	public static function activate($data, $key) {
		...
		if ( is_wp_error( $response ) ) {
			...
			if ( is_wp_error( $response) ) {
				echo '<div class="error"><p>' . NBMCore::__( 'When attempting to activate license, could not reach license site.' ) . '</p></div>';
		...
		if ( ! $response) {
			echo '<div class="error"><p>' . NBMCore::__('When attempting to activate license, no response was received.') . '</p></div>';
		} else {
			if (wp_remote_retrieve_response_code($response) != 200) {
				echo '<div class="error"><p>' . sprintf(NBMCore::__('When attempting to activate license, a response code of %d was returned.'), wp_remote_retrieve_response_code($response)) . '</p>';
				...
				if ( $response->license != 'valid' ) {
					echo '<div class="error"><p>' . sprintf(NBMCore::__( 'The license entered for %s is invalid.' ), $item_name) . '</p></div>';
					$key = array(
						'key'     => '',
						'expires' => NULL,
						'valid'   => FALSE
					...
					echo '<div class="updated"><p>' . sprintf(NBMCore::__( 'Congratulations! The %s license entered is valid.' ), $item_name);
					if ($item_key != 'core') {
						echo ' <a href="' . admin_url('admin.php?page=manage_settings') . '">' . sprintf(NBMCore::__('(Click to refresh if %s is not visible.)'), $item_name) . '</a>';
					}
					echo '</p></div>';
					$key = array(
						'key'     => $key,
			...
			$all_reg_info = NBMCore::get_reg_info();
			$all_reg_info[$item_key] = $key;
			update_option('nbm_license', $all_reg_info);
		}		
	}

	// This method is used for checking add-on status, etc.
	public static function make_call($method) {
		$params = array('api_call'  => json_encode(array('method'   => 'nbm_' . $method)));
		$response = '';
