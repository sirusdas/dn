...
abstract class NBMCore {

	const VERSION = '1.2.0';

	const SHORTCODE = 'netrabilling';

	const SETTINGS = 'netrabilling_settings';
	const SETTINGS_GROUP = 'netrabilling_settings_group';

	const VIEWFOLDER = 'netrabilling/views/';
	const LANG = 'netrabilling';
	const MENU = 'netrabilling';
	const NONCE_ACTION = 'netrabilling_&%k2s$%#!@#8vY^';

	protected static $filters = array(
                "search_from_date"      => "search_from",
	...
	public function __construct() {
		...
		self::$label        = NBMLabel::getInstance();
	...
	public static function store_page_state( $is_single = FALSE ) {
		...
		if ( $update_transient ) {
			...
			$_SESSION['netrabilling_page_state'] = $page_state;
		}
	}

	public static function get_page_state() {
		return ( isset( $_SESSION['netrabilling_page_state'] ) ) ? (array) $_SESSION['netrabilling_page_state'] : array();
	}
...
//	}
	...
	public static function get_reg_info() {
		$reg_info = get_option( 'nbm_license' );

		if ( $reg_info && ! isset( $reg_info['core'] ) ) {
			$reg_info = array( 'core' => $reg_info );
			update_option( 'nbm_license', $reg_info );
		}
	...
	protected static function get_add_ons( $installed = FALSE, $force = FALSE ) {
		$add_ons = get_transient( 'nbm_add_ons' );
		...
		if ( ! $add_ons ) {
			$add_ons = NBMAPI::make_call( 'get_add_ons' );
			set_transient( 'nbm_add_ons', $add_ons, 12 * HOUR_IN_SECONDS );
		}

		$add_ons = apply_filters( 'nbm_add_ons_list', $add_ons );
	...
	protected static function user_can_edit( $inventory_id = NULL ) {
		return NBMAdmin::check_permission( 'edit_item', $inventory_id );
	}
	...
	protected static function check_permission( $type, $inventory_id ) {
		...
		return apply_filters( 'nbm_check_permission', TRUE, $type, $inventory_item );
	}
