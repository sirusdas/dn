 ...
 *
 */
class NBMShortcode extends NBMCore {
	...
	public function get($args = array()) {
		...
		if (self::$args['category_id'] && ! (int)self::$args['category_id']) {
			$NBMCategory = new NBMCategory();
			$category = $NBMCategory->get_id_from_name(self::$args['category_id']);
			if ($category) {
		...
		self::$loop = new NBMLoop(self::$args);
		netrabilling_set_loop(self::$loop);
	...
	public function get_template($template, $echo = FALSE, $type = 'list') {
		...
		if ( ! file_exists($file)) {
			echo '<!-- NETRABilling Error: template part ' . $file . ' missing! -->';
		}
		...
		do_action('nbm_get_template', $template, $echo, $type, self::$inventory_id);
