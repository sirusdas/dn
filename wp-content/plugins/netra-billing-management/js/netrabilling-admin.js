jQuery(function($) {
	...
	$('.inventorywrap a.delete').click(
		function() {
			var prompt = netrabilling.delete_prompt;
			var name = $(this).attr("data-name");
			if (typeof name != "undefined" && name != "") {
				prompt+= ' ' + netrabilling.delete_named + '"' + name + '"' + netrabilling.prompt_qm;
			} else {
				prompt+= ' ' + netrabilling.delete_general + netrabilling.prompt_qm;
			}
	 ...
	 */
	NETRABillingUpload.init();
});

/**
 * The image / media upload class
 */
var NETRABillingUpload = {};

NETRABillingUpload = (function() {
	var $;
	var container;
	var type;
	var word;
	...
	function renderMedia(field, src) {
		...
		var html = '<p><label>' + netrabilling.title_label + ':</label><input type="text" class="widefat" name="media_title[' + field + ']" value="" /></p>'; 
		html+= '<p class="media_url"><label>' + netrabilling.url_label + '</label><span>' + src + '</span></p><a href="javascript:void(0);" data-count="' + field + '" class="delete">X</a>';
	...
	function ensureNewImage() {
		...
		if ( ! empty) {
			...
			html+= '<a href="media-upload.php?post_id=0&type=image&TB_iframe=1&width=640&height=673" data-count="' + count + '" id="inventory-link-' + count + '" class="netrabilling-upload">Add New Image</a>';
		...
		$('.mediasortable').each(function() {
			...
			if (count > 1) {
				if ($(this).next(".sortnotice").length <= 0) {
					word = ($(this).attr("data-type") == 'media') ? netrabilling.media_label : netrabilling.image_label;
	...
	return {
		init: function() {
			...
			$(document).on('click', '.netrabilling-upload', function() {
				...
				word = (type == 'media') ? netrabilling.media_label : netrabilling.image_label;
				inventoryUpload($(this));
				return false;
			});
